...
void temp(int particleNumber)
{
	for (int i = 0; i < particleNumber; i++) {
		particles.push_back(new Particle(XRandom(),-700,0));
	}
}
...
int main(int argc, char** argv)
{
	...
	t_particle1 = TextureManager::Inst()->LoadTexture("C:/Users/Fab/Desktop/XD/Computacion Grafica/FINAL/Models/smoke1.png", GL_BGRA_EXT, GL_RGBA);
...
void GenerateParticles(double dt,int particleNumber) {
	...
	for (itP = particles.begin(); i<particleNumber; itP++) {
		if ((*itP)->getT() > 0.0) {
            ...
            (*itP)->restartParticle(XRandom(),-700,0);
			//delete(*itP);
